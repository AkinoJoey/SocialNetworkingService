...
#### 署名付きURLによる検証
ユーザーが入力したEメールが不正ではないかどうか確かめるEメール認証機能を実装しました。
これにより不正アカウントの作成を防ぐことができます。

[アカウント作成]()のページでユーザーが情報を入力し、送信すると、署名付きURLが作成され、入力されたEメール宛に送られます。
ユーザーが届いた署名付きURLを開くと、Eメールが有効とみなされアカウントが作成されます。

URLの署名にはサーバーサイドのシークレットキーとメッセージハッシュアルゴリズム（HMAC SHA256）を使用しました。
署名付きURLにはユーザーID、ハッシュ化されたEメール、有効期限を含めました。

サーバーが署名URLへのリクエストを受け取ると、現在のURLをシークレットキーでハッシュし、URLに埋め込まれた署名と比較して署名が有効であることを確認します。
署名が有効であることを確認できたら、ユーザーはアカウント作成に成功し、ログインができます。
