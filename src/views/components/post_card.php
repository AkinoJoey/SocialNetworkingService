    ...
    <div class="flex flex-wrap justify-between">
        <div class="flex space-x-2 text-sm dark:text-gray-400">
            ...
            <button type="submit" name="post_like_btn" class="like-btn z-40 flex items-center space-x-1.5 p-1 hover:bg-blue-100 rounded-full" data-post-id="<?= $post->getId() ?>" data-isLike="<?= $post->getIsLike() ?>">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" aria-label="Number of likes" class="good-btn h-4 w-4 dark:text-violet-400 <?php if ($post->getIsLike() === 1) echo 'fill-blue-600' ?> ">
                ...
                <span class="number-of-post-likes"><?= $post->getNumberOfLikes() ?></span>
            </button>
        </div>
    </div>
</div>